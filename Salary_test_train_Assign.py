# -*- coding: utf-8 -*-
"""
Created on Mon Jan 29 15:22:35 2024
...
#1.) Prepare a classification model using the Naive Bayes algorithm
# for the salary dataset. Train and test datasets are given
# separately. Use both for model building

'''Objective: Predict whether an individual's salary is above a certain threshold.'''

'''Maximize:
# Maximize the accuracy of the model to correctly predict whether an individual's salary is above the threshold.
# Maximize precision if false positives are costly.'''

'''Minimize:
Minimize the misclassification rate to reduce errors in predicting salary categories.
Minimize false negatives if missing potential high-salary individuals is costly.'''
...
Test_data=pd.read_csv('c:/10-ML/NaiveBayes/SalaryData_Test.csv')
Test_data.head()
...
y=Test_data[['Salary']]
###########################################
Train_data=pd.read_csv('c:/10-ML/NaiveBayes/SalaryData_Train.csv')
Train_data
############################################
